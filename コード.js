...
function ensurePrioritySheet_() {
  const props = PropertiesService.getScriptProperties();
  if (props.getProperty('PRIORITY_SHEET_DONE_V2')) return;
  if (getSS_().getSheetByName('優先順位')) { props.setProperty('PRIORITY_SHEET_DONE_V2', '1'); return; }
  withLock_(function () { if (!getSS_().getSheetByName('優先順位')) createPrioritySheet_(); });
  props.setProperty('PRIORITY_SHEET_DONE_V2', '1');
}
